/*
 * This implementation of FABIA runs with sparse input matrices.
 *
 * These matrices are stored in CSR format. More specifly, it uses the
 * 3-Array variation of the format as described in the MKL Library Reference:
 * http://software.intel.com/sites/products/documentation/hpc/mkl/mklman/index.htm
 *
 * This code requires the MKL (and uses its Sparse BLAS functions).
 * 
 */
...
char MATDESCRX[6] = {'G', 0, 0, 0, 0, 0}; // needed in sparse BLAS, describes general matrix
 ...
 * calculated.
 */
static inline void sparse_estimateZ(int n, int k, float* restrict X_values,
    int* restrict X_columns, int* restrict X_rowIndices, int j,
    float* z, float* lapla,
    float* LPsi, float* LPsiL, float* tLPsiL, float* iLPsiLX,
	float* sum2, float spz, float lap)
{
    ...
    const int nnz = X_rowIndices[j+1] - X_rowIndices[j];
    const int offset = X_rowIndices[j];
    memset(z, 0, k*sizeof(float));
    for (int j = 0; j < nnz; ++j) {
        float x = X_values[offset + j];
        int r = X_columns[offset + j];
        for (int i = 0; i < k; ++i) {
            z[i] += x * iLPsiLX[r*k + i];
        }
    }
...
/**
 * Runs the main FABIA algorithm. Expects X in CSR format (3 array variation)
 * and all other matrices in column-major layout. Operates on float matrices.
 * 
 * @param X_values: values for all nonzero values in X (length: nnz)
 * @param X_columns: position i stores the column of the i-th entry in X_values (length: nnz)
 * @param X_rowIndex: position j gives the index of the first element in X_values from row j (length: l+1)
 ...
 */
void sparse_fabia_cm_f(int k, int n, int l, const int nnz,
	float* restrict X_values, int* restrict X_columns, int* restrict X_rowIndices,
    float* restrict Psi, float* restrict L, float* restrict Z, float* restrict lapla,
	int cyc, float alpha, float eps, float spl, float spz, int scale, float lap, int verbose, int nthreads)
{
	...
	float *iLPsiLX_ = (float*) malloc(nthreads * k * n * sizeof(float));
    ...
    */
    for (int i = 0; i < nnz; ++i) {
        XX[X_columns[i]] += X_values[i] * X_values[i];
    }
	for (int i2 = 0; i2 < n; ++i2) {
		XX[i2] /= l;
        XX[i2] = fmax(XX[i2], eps);
    }
	...
	for (int iter = 1; iter <= cyc; ++iter) {
		...
		for (int j = 0; j < l; ++j) {
			...
			sparse_estimateZ(n, k, X_values, X_columns, X_rowIndices, j, Z+j*k, lapla+j*k, LPsi, LPsiL, tLPsiL,iLPsiLX, sum2, spz, lap);
		}	
        ...
        mkl_simatcopy('C', 'T', k, l, 1.0, Z, l, k); // transpose Z
        mkl_scsrmm("n", &n, &k, &l, &ONE, MATDESCRX, X_values, X_columns, X_rowIndices, X_rowIndices+1, Z, &k, &ZERO, XZ, &n);
	...
	if (t >= eps ) {
		...
		for (int j = 0; j < l; ++j) {
			...
			float* tLPsiL = tLPsiL_ + (tid*k*k);
			float* iLPsiLX = iLPsiLX_ + (tid*n*k);
			sparse_estimateZ(n, k, X_values, X_columns, X_rowIndices, j, Z+j*k, lapla+j*k, LPsi, LPsiL, tLPsiL,iLPsiLX, NULL, spz, lap);
		}
	}
	else
		memset(Z, 0, k*l*sizeof(float));
